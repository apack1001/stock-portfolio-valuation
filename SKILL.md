...
description: 从持仓CSV或本地截图读取持仓数据，实时获取最新行情，自动计算总估值、盈亏金额和盈亏比例，生成完整持仓报告。支持美股、港股、A股、基金（支付宝/腾讯理财通/富途理财）。当用户提到"看一下我的持仓"、"帮我估值"、"我的股票现在多少钱"、"持仓盈亏"、"买入"、"卖出"等时触发。
---
# 股票持仓实时估值
## 数据源
...
## 执行步骤
### Step 1：确定数据来源
**A. CSV 已存在（正常路径）**：直接进入 Step 2，跳过图片读取。
**B. 用户提供了新截图，或 CSV 不存在**：
- 用 Read 工具读取图片，识别字段
  - 股票：名称、代码、市场、持仓数量、成本均价
  - 基金：名称、当前市值（持有金额）、持仓收益
- 按格式追加/更新 CSV，保存后进入 Step 2
---
